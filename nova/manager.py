...
class ComputeManager(manager.Manager):
    ...
    def _prep_block_device_cacca(self, context, instance, bdms):
        """Set up the block device for an instance with error logging."""
        try:
            print 'whr [_prep_block_device_cacca] start'
    ...
    def _prep_block_device(self, context, instance, bdms):
        """Set up the block device for an instance with error logging."""
        try:
            print 'whr [_prep_block_device] start'
            self._add_missing_dev_names(bdms, instance)
            block_device_info = driver.get_block_device_info(instance, bdms)
            mapping = driver.block_device_info_get_mapping(block_device_info)
            #ucciso
    ...
    def build_and_run_instance(self, context, instance, image, request_spec,
        ...
        print 'whr [bbbbbbb] time %s' % time.time()
    ...
    def _build_and_run_instance(self, context, instance, image, injected_files,
        ...
        print 'whr [buid_and_run_instance] ### START ### %0.06f '% time.time()
        ...
        print 'whr [buid_and_run_instance] init %0.06f ' % (time.time() - time_prev)
        time_prev = time.time()
        ...
        try:
            rt = self._get_resource_tracker()
            print 'whr [buid_and_run_instance]  prima del claim'
            with rt.instance_claim(context, instance, node, limits):
                ...
                print 'whr [buid_and_run_instance] claim  %0.06f ' % (time.time() - time_prev)
                time_prev = time.time()
                with self._build_resources(context, instance,
                        requested_networks, security_groups, image_meta,
                        block_device_mapping) as resources:
                    instance.vm_state = vm_states.BUILDING
                    print 'whr [buid_and_run_instance] build res %0.06f ' % (time.time() - time_prev)
                    time_prev = time.time()
                    ...
                    print 'whr [buid_and_run_instance] spawning instance'
                    spawn_start = time.time()
                    print 'whr [buid_and_run_instance] ### Spawn start ### %0.06f '% spawn_start

                    print 'whr [buid_and_run_instance] instance save %0.06f ' % (time.time() - time_prev)
                    time_prev = time.time()
                    ...
                    print 'whr [buid_and_run_instance] spawing on the hypervisor %0.06f' % timer.elapsed()
 
                    LOG.info('Took %0.2f seconds to spawn the instance on '
                             'the hypervisor.', timer.elapsed(),
                             instance=instance)
                    print 'whr [buid_and_run_instance] ### Spawn end ### %0.06f '% time.time()
                    print 'whr [buid_and_run_instance] ==> Spawn time ### %0.06f '% (time.time() - spawn_start)
        ...
        print 'whr [buid_and_run_instance] ## end ##  %0.06f ' % (time.time())
    ...
    def _build_resources(self, context, instance, requested_networks,
        ...
        try:
            ...
            print 'cacca unespected task state error'
            raise exception.BuildAbortException(instance_uuid=instance.uuid,
                    reason=e.format_message())
        except Exception:
        ...
        try:
            ...
            if image_meta.container_format == 'uki':
                print 'cacca ukiiiiiiiiiii'
                block_device_info = {}#self._prep_block_device_cacca(context, instance,
                    #block_device_mapping)
            else:
