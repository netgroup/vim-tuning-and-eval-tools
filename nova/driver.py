...
class LibvirtDriver(driver.ComputeDriver):
    ...
    def spawn(self, context, instance, image_meta, injected_files,
        ...
        disk_info = blockinfo.get_disk_info(CONF.libvirt.virt_type,
                                            ...
                                            image_meta,
                                            block_device_info)
        print 'whr [spawn] ### START ## %s' % time.time()
        
        time_ref = time.time()
        time_prev = time_ref
        time_create_img = time_ref
        

        injection_info = InjectionInfo(network_info=network_info,
                                       files=injected_files,
                                       admin_pass=admin_password)
        print 'whr [spawn] injection info  %0.06f' % (time.time() - time_prev)
        time_prev = time.time()
        ...
        gen_confdrive = functools.partial(self._create_configdrive,
                                          context, instance,
                                          injection_info)
        print 'whr [spawn] gen_confdrive  %0.06f' % (time.time() - time_prev)
        time_prev = time.time()
        ...
        self._create_image(context, instance, disk_info['mapping'],
                           ...
                           block_device_info=block_device_info)
        print 'whr [spawn] ==> create img %0.06f' % (time.time() - time_create_img )
        time_prev = time.time()
        time_xml = time_prev
        ...
        print 'whr [spawn] console log  %0.06f' % (time.time() - time_prev)
        time_prev = time.time()
        ...
        print 'whr [spawn] ==> get guest xml  %0.06f' % (time.time() - time_xml)
        time_prev = time.time()
        time_libvirt = time_prev
        ...
        print 'whr [spawn] domain and network  %0.06f' % (time.time() - time_prev )
        ...
        timer.start(interval=0.5).wait()
        print 'whr [spawn] wait for boot  %0.06f' % (time.time() - time_prev)
        print 'whr [spawn]  %0.06f' % (time.time() - time_ref)
        print 'whr [spawn] ==> libvirt spawn  %0.06f' % (time.time() - time_libvirt)
        print 'whr [spawn] ### END ## %s' % time.time()
    ...
    def _create_image(self, context, instance,
        ...
        print 'whr [create image] ### create image start ###'
        ...
        print 'whr [create_image] write test %0.06f' % (time.time() - time_prev)
        time_prev = time.time()
        ...
        print 'whr [create_image] dopo if swap %0.06f' % (time.time() - time_prev)
        time_prev = time.time()
        ...
        if not disk_images:
            disk_images = {'image_id': instance.image_ref,
                           'kernel_id': instance.kernel_id,
                           'ramdisk_id': instance.ramdisk_id}
        print 'whr [create_image] dopo if not disk img %0.06f' % (time.time() - time_prev)
        time_prev = time.time()

        if disk_images['kernel_id']:
            fname = imagecache.get_cache_fname(disk_images['kernel_id'])
            if instance.image_meta.container_format == 'uki':
                raw('kernel').cache(fetch_func=libvirt_utils.fetch_raw_image,
                                context=context,
                                filename=fname,
                                image_id=disk_images['kernel_id'],isuki=True)
            else:
                raw('kernel').cache(fetch_func=libvirt_utils.fetch_raw_image,
                                context=context,
                                filename=fname,
        ...
        print 'whr [create_image] dopo get cache  %0.06f' % (time.time() - time_prev)
        time_prev = time.time()


        if CONF.libvirt.virt_type == 'uml':
            libvirt_utils.chown(image('disk').path, 'root')
        print 'whr [create_image] dopo chown image %0.06f' % (time.time() - time_prev)
        time_prev = time.time()
        ...
        if instance.image_meta.container_format != 'uki':
            self._create_and_inject_local_root(context, instance,
                                               ...
                                               fallback_from_host)
        print 'whr [create_image] create and inject %0.06f' % (time.time() - time_prev)
        time_prev = time.time()
        ...
        print 'whr [create_image] fs lookup %0.06f' % (time.time() - time_prev)
        time_prev = time.time()
        ...
        for idx, eph in enumerate(driver.block_device_info_get_ephemerals(
            ...
            disk_image.cache(fetch_func=fn,
                             ...
                             specified_fs=specified_fs)
            print 'whr [create_image] dopo ephemeral %0.06f' % (time.time() - time_prev)
        time_prev = time.time()
        ...
        if swap_mb > 0:
            ...
            image('disk.swap').cache(fetch_func=self._create_swap,
                                     ...
                                     size=size,
                                     swap_mb=swap_mb)
        print 'whr [create_image] dopo swap %0.06f' % (time.time() - time_prev)
        time_prev = time.time()
    ...
    def _get_guest_config(self, instance, network_info, image_meta,
        ...
        #whr uccido
        if image_meta.container_format == 'uki':
            storage_configs = {}
        else:
    ...
    def _create_domain_and_network(self, context, xml, instance, network_info,
        ...
        block_device_mapping = driver.block_device_info_get_mapping(
            block_device_info)
        print 'whr [create_domain_and_network] ### start ###'
        print 'whr [create_domain_and_network] bld mapping %s' % str(block_device_mapping)
        #uccisa
        if instance.image_meta.container_format == 'uki':
            print 'whr [create_domain_and_network] driver ukiiii crate'
        else:
            for vol in block_device_mapping:
                connection_info = vol['connection_info']
                print 'whr [create_domain_and_network] vol %s' % str(connection_info)
                print 'whr [create_domain_and_network] itero block_device_maping'
