...
image_uuid = '18eae1ae-76e4-4a6c-8bd8-382e599928c2'
...
class LibvirtDriver(driver.ComputeDriver):
    ...
    def _hard_reboot(self, context, instance, network_info,
        ...
        #             are in place.
        xml_s_time = time.time()
        xml = self._get_guest_xml(context, instance, network_info, disk_info,
                                  image_meta,
                                  block_device_info=block_device_info,
                                  write_to_disk=True)
        xml_e_time = time.time()
        ...
        # start the instance.
        create_s_time = time.time()
        self._create_domain_and_network(context, xml, instance, network_info,
                                        disk_info,
                                        block_device_info=block_device_info,
                                        reboot=True,
                                        vifs_already_plugged=True)
        create_e_time = time.time()
        pci_s_time = time.time()
        self._prepare_pci_devices_for_use(
            pci_manager.get_instance_pci_devs(instance, 'all'))
    ...
    def spawn(self, context, instance, image_meta, injected_files,
        ...
        timer.start(interval=0.5).wait()
        CLOG.info("spawn end %f %f %f",(xml_s_time - s_time),(xml_e_time - xml_s_time),(e_time - s_time))
    ...
    def _get_guest_config_ka(self, instance, network_info, image_meta,
                          disk_info, rescue=None, block_device_info=None,
                          context=None):
        """Get config data for parameters.
    ...
    def _clickosize(self,xml_string):

        root = etree.fromstring(xml_string)

        for elem in root.findall('features'):
            root.remove(elem)

        elem = root.find('./devices')
        for sub_elem in elem.findall('disk'):
            elem.remove(sub_elem)
        for sub_elem in elem.findall('memballoon'):
            elem.remove(sub_elem)
        for sub_elem in elem.findall('graphics'):
            elem.remove(sub_elem)
        for sub_elem in elem.findall('video'):
            elem.remove(sub_elem)

        for sub_elem in elem.findall('interface'):
            elem.remove(sub_elem)

        iface = etree.SubElement(elem, "interface")
        iface.set("type","bridge")

        mac = etree.SubElement(iface, "mac")
        randomaddr = "fa:16:3e:%02x:%02x:%02x" % (
                                                random.randint(0,255),
                                                random.randint(0,255),
                                                random.randint(0,255)
                                                )
        mac.set("address", randomaddr)

        source = etree.SubElement(iface, "source")
        source.set("bridge", "br100")


        elem = root.find('./os')
        elem.find('kernel').text = '/root/clickos_x86_64'
        elem.remove(elem.find('initrd'))
        print 'clickosme %s' %(time.time())
        r = etree.tostring(root,pretty_print=True)
        #print 'clickosxml %s' % r
        return r
    ...
    def _get_guest_xml(self, context, instance, network_info, disk_info,
        ...
        #  network_info_str = '' #str(network_info) whr
    ...
    def _create_domain(self, xml=None, domain=None,
        ...
        """
        create_domanin_s_time = time.time()
        if xml:
	    if image_uuid in xml:
	        xml = self._clickosize(xml)
    ...
    def _create_domain_and_network(self, context, xml, instance, network_info,
        ...
        try:
            with self.virtapi.wait_for_instance_event(
                ...
                #self.plug_vifs(instance, network_info) #uccidimi inizio
